...
@property (nonatomic) CGFloat finalDragVelocity;
...
- (void)reloadData
{
    ...
    for (int i = begin; i<= end;i++) {
        ...
        [self.delegate updateView:self.scrollDirection num:self.num];
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    for (UIView *view in self.views) {
        ...
        CGFloat velocity = -([scrollView.panGestureRecognizer velocityInView:self.scrollView].x);
        
        if(velocity != 0)
        {
            self.finalDragVelocity = velocity;
        }
        
        if (self.scrollDirection == ScrollDirectionLeft)
        {
            if(self.finalDragVelocity >= 100)
            {
                self.num += 5;
            }
            else if(self.finalDragVelocity >= 300)
            {
                self.num += 15;
            }
            else if(self.finalDragVelocity >= 500)
            {
                self.num += 30;
            }
            else if(self.finalDragVelocity >= 1000)
            {
                self.num += 50;
            }
            else
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
    ...
    if(velocity.x > 0)
    {
        if(velocity.x < 3)
        {
            self.finalDragVelocity = 100;
        }
        else if (velocity.x < 5)
        {
            self.finalDragVelocity = 500;
        }
        else
        {
            self.finalDragVelocity = 1000;
        }
    }
    else
    {

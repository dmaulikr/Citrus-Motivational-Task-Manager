...
-(void)didTapOnBackgroundView:(UITapGestureRecognizer *)recognizer
{
...
- (void)showAlertView
{
    ...
    _vAlert.backgroundColor = [UIColor clearColor];//DO_RGBA(255, 255, 255, 0.7);
    ...
    if (_strAlertTitle != nil && _strAlertTitle.length > 0)
    {
        ...
        if (_bDestructive)
            vTitle.backgroundColor = [UIColor clearColor];//DO_DESTRUCTIVE_BOX;
        else
            vTitle.backgroundColor = [UIColor clearColor];//DO_TITLE_BOX;
        vTitle.backgroundColor = [UIColor clearColor];//[UIColor whiteColor];
        ...
        dHeight = vTitle.frame.size.height + 10;
    }
    ...
    if (_bDestructive)
        ...
        vBody.backgroundColor = [UIColor clearColor];//DO_NORMAL_BOX;
    ...
    [lbBody setFont:[UIFont fontWithName:@"Bariol_Regular" size:17]];
    ...
    if (_doNo != nil)
    {
        ...
        btNo.frame = CGRectMake(0, dHeight+60, _vAlert.frame.size.width / 2.0 - 1, 40);
        //btNo.backgroundColor = DO_BUTTON_BOX;
        btNo.tag = DO_NO_TAG;
        [btNo setImage:[UIImage imageNamed:@"xmark.png"] forState:UIControlStateNormal];
    ...
    if (_doYes != nil)
    {
        ...
        if (_doNo == nil)
            btYes.frame = CGRectMake(0, dHeight+60, _vAlert.frame.size.width, 40);
        else
            btYes.frame = CGRectMake(_vAlert.frame.size.width / 2.0 - 0.5, dHeight+60, _vAlert.frame.size.width / 2.0 + 0.5, 40);

        //btYes.backgroundColor = DO_BUTTON_BOX;
        btYes.tag = DO_YES_TAG;
        [btYes setImage:[UIImage imageNamed:@"checkmark.png"] forState:UIControlStateNormal];
...
- (double)addContent:(UIView *)vBody
{
    ...
    switch (_nContentMode) {
        case DoContentPicker:
        {
            self.datePicker = [[UIDatePicker alloc] initWithFrame:CGRectMake(DO_LABEL_INSET.left,DO_LABEL_INSET.top-5, 330, 200)];
            self.datePicker.tintColor = [UIColor whiteColor];
            self.datePicker.backgroundColor = [UIColor clearColor];
            self.datePicker.center = CGPointMake(vBody.center.x, self.datePicker.center.y);
            self.datePicker.datePickerMode = UIDatePickerModeTime;
            [vBody addSubview:self.datePicker];
            dContentOffset = 110 + DO_LABEL_INSET.bottom;
        }
            break;
        default:
            break;
...
- (void)setLabelAttributes:(UILabel *)lb
{
    ...
    if (_bDestructive)
    {
        ...
        lb.textColor = [UIColor whiteColor];//DO_DESTRUCTIVE_TEXT_COLOR;
    }
    else
    {
        lb.font = DO_TEXT_FONT;
        lb.textColor = [UIColor whiteColor];//DO_TEXT_COLOR;
    }
}
